  ...
  <div class="container">
    <form>
      <div class="total">
        ...
        <p><textarea type="text" id="content" v-model="content" style="width:80%; padding:10px; resize:none;" rows=15px></textarea></p>
...
export default {
  ...
  methods:{
    onClick(){
      ...
      const movie_id = this.$store.state.selectMovie.movie_id
      const URL =  `http://127.0.0.1:8000/review/${movie_id}/`
      const params = {
        method: 'post',
        url: URL,
        data: data,
  ...
  computed:{
    ...
    posterPath(){
      return 'https://image.tmdb.org/t/p/w500/' + this.$store.state.selectMovie.poster_path
    },
  }
}
</script>
